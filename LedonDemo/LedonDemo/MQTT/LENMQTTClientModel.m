...
@interface LENMQTTClientModel()<LENWifiManagerDelegate, MQTTSessionDelegate>
...
@implementation LENMQTTClientModel

+ (instancetype)sharedInstance {
    static LENMQTTClientModel *user = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        user = [[LENMQTTClientModel alloc]init];
        [LENWifiManagerInstance addDelegate:user];
...
- (void)manager:(LENWifiManager *)manager reachabilityChanged:(LENNetworkStatus)status {
    if (status == LENNetworkStatusNotReachable) {
        [self disconnect];
#warning 网络改变所以设备离线
    }
