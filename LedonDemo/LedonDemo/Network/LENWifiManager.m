...
@property (weak, nonatomic) id<LENWifiManagerDelegate> delegate;

- (instancetype)initWithDelegate:(id<LENWifiManagerDelegate>)delegate;
...
@interface LENWifiManager ()
{
    LENReachability    *reachability;  // 可达性
    NSMutableArray  *delegateQueue;
}

@end

@implementation LENWifiManager

/**
 * 单例方法
 */
+ (instancetype)sharedInstance {
    static LENWifiManager *wifiManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        wifiManager = [[LENWifiManager alloc] init];
    });
    return wifiManager;
}
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        reachability = [LENReachability reachabilityForLocalWiFi];
        // 注册通知
...
-(BOOL)isWifiEnable {
    return reachability.currentReachabilityStatus == LENNetworkStatusReachableViaWiFi;
}
