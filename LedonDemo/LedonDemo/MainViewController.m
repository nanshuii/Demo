...
- (void)viewDidLoad {
    ...
    self.titles = @[
  @[@"文件分享与快速预览", @"接收文件分享", @"MQTT Demo", @"蓝牙", @"蓝牙example" ,@"二维码配网"],
  @[@"方块网格选择", @"二维码的生成和扫描", @"通知设定", @"进度条" , @"方向按键", @"转盘", @"颜色的选择", @"banner", @"验证码输入", @"文字TableView"],
  @[@"自定义的TableViewCell"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        LENProgressViewController *vc = [LENProgressViewController new];
        [self.navigationController pushViewController:vc animated:YES];
        ...
        LENBannerViewController *vc = [LENBannerViewController new];
        [self.navigationController pushViewController:vc animated:YES];
    } else if (indexPath.section == 1 && indexPath.row == 8) {
        LENCodeTextViewController *vc = [LENCodeTextViewController new];
        [self.navigationController pushViewController:vc animated:YES];
    } else if (indexPath.section == 1 && indexPath.row == 9) {
        LENTextTableViewController *vc = [LENTextTableViewController new];
        [self.navigationController pushViewController:vc animated:YES];
    }
